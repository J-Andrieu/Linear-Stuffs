...
__kernel void addScalar(__global const float* A, __global float* B, __global const float* val) {
	size_t index = get_global_id(0);
	B[index] = A[index] + val[0];
}

__kernel void multiplyScalar(__global const float* A, __global float* B, __global const float* val) {
	size_t index = get_global_id(0);
	B[index] = A[index] * val[0];
}

__kernel void subtractScalar(__global const float* A, __global float* B, __global const float* val) {
	size_t index = get_global_id(0);
	B[index] = A[index] - val[0];
}

__kernel void divideScalar(__global const float* A, __global float* B, __global const float* val) {
	size_t index = get_global_id(0);
	B[index] = A[index] / val[0];
}
